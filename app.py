...
with tab2:
    ...
    try:
        ...
        # Handle pyaudio not available (Streamlit Cloud)
        st.markdown("""
        <div class="warning-box">
        <h4>🎤 Voice Input Note</h4>
        <p><b>Direct microphone access requires additional setup:</b></p>
        <ol>
            <li><b>For local use:</b> Install PyAudio: <code>pip install pyaudio</code></li>
            <li><b>For Streamlit Cloud:</b> Use Text Input or Image Upload methods</li>
        </ol>
        <p>In the meantime, you can:</p>
        </div>
        """, unsafe_allow_html=True)
        
        # Provide alternative text input for voice
        st.markdown("#### Type what you want to say:")
        alt_voice = st.text_area("Simulate voice input:", height=100, key="alt_voice")
        
        if st.button("Use as Voice Input", key="alt_voice_btn", use_container_width=True):
            if alt_voice.strip():
                st.session_state.input_text = alt_voice
                st.session_state.detected_language = detect_language(alt_voice)
                st.session_state.translation_ready = True
